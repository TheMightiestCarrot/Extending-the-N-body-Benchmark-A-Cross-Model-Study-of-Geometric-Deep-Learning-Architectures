...
from datasets.nbody.visualization_utils import calculate_centre_of_mass
...
def get_pbc_distances(
    ...
    offsets = cell_offsets.view(-1, 1, 3).bmm(cell).view(-1, 3)
...
def radius_graph_pbc(
    data, radius, max_num_neighbors_threshold, batch_size, num_atoms, pbc=[True, True, True]
):
    device = data.batch.device
    natoms = data.natoms if "natoms" in data else torch.tensor([num_atoms] * batch_size, device=device)
    ...
    atom_pos = data.pos if "pos" in data else data[0]
...
def merge_dicts(dict1: dict, dict2: dict):
    ...
    Additionally returns a list of detecteqd duplicates.
...
def compute_neighbors(data, edge_index, batch_size, num_atoms):
    ...
    natoms = data.natoms if "natoms" in data else torch.tensor([[num_atoms]] * batch_size, device=data[0].device) 
...
def calculate_direction_to_com(pos, n_bodies):
    """
    Calculate direction to com for each body.
    We assume that all bodies have equal mass.
    We then normalize directionnal vectors.
    """
    batch_size = int(pos.shape[0]/ n_bodies)
    pos = pos.cpu().detach().numpy()
    arr = []
    for batch in range(batch_size):
        start_idx = batch*n_bodies 
        end_idx = start_idx+n_bodies
        batch_pos = pos[start_idx:end_idx]
        centre_of_mass = calculate_centre_of_mass(batch_pos)
        direction_vectors = torch.tensor(centre_of_mass - batch_pos)
        norms = torch.norm(direction_vectors, dim=1, keepdim=True)
        normalized_directions = direction_vectors / norms
        arr.append(normalized_directions)
    normalized_directions = torch.cat(arr)
