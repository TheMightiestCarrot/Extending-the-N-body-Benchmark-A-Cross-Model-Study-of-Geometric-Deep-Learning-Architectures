...
class EdgeDegreeEmbedding(torch.nn.Module):
    ...
    def __init__(
        ...
        self.edge_channels_list.append(
            (self.m_0_num_coefficients * self.sphere_channels).item()
        )
    ...
    def forward(self, atomic_numbers, edge_distance, edge_index):
        ...
        x_edge_m_0 = x_edge_m_0.reshape(
            -1, self.m_0_num_coefficients.int().item(), self.sphere_channels
        )
        x_edge_m_pad = torch.zeros(
            (
                x_edge_m_0.shape[0],
                (self.m_all_num_coefficents - self.m_0_num_coefficients.int().item()),
