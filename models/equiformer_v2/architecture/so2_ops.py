...
class SO2_Convolution(torch.nn.Module):
    ...
    def forward(self, x, x_edge):
        ...
        x_0 = x.embedding.narrow(1, 0, self.mappingReduced.m_size[0].int().item())
        ...
        offset = self.mappingReduced.m_size[0].int()
        for m in range(1, max(self.mmax_list) + 1):
            # Get the m order coefficients
            x_m = x.embedding.narrow(
                1, offset.int().item(), 2 * self.mappingReduced.m_size[m].int().item()
            )
