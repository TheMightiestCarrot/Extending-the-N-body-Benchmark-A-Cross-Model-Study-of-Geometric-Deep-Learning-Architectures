...
class CoefficientMappingModule(torch.nn.Module):
    ...
    def __init__(
        ...
        l_harmonic = torch.tensor([], device=self.device)
        m_harmonic = torch.tensor([], device=self.device)
        m_complex = torch.tensor([], device=self.device)
        ...
        for i in range(self.num_resolutions):
            for lval in range(0, self.lmax_list[i] + 1):
                ...
                m = torch.arange(-mmax, mmax + 1, device=self.device)
                m_complex = torch.cat([m_complex, m], dim=0)
                m_harmonic = torch.cat(
                    [m_harmonic, torch.abs(m)], dim=0
                )
                l_harmonic = torch.cat(
                    [l_harmonic, m.fill_(lval)], dim=0
                )
    ...
    def complex_idx(self, m: int, lmax: int, m_complex, l_harmonic):
        ...
        mask_idx_i = torch.tensor([], device=self.device)
        # Imaginary part
        if m != 0:
...
class SO3_Rotation(torch.nn.Module):
    ...
    def RotationToWignerDMatrix(
        ...
        wigner = torch.zeros(len(alpha), size, size, device=self.device, dtype=edge_rot_mat.dtype)
        start = 0
...
class SO3_LinearV2(torch.nn.Module):
    def __init__(
        ...
        expand_index = torch.zeros([(lmax + 1) ** 2])
        for lval in range(lmax + 1):
            start_idx = lval**2
            length = 2 * lval + 1
    ...
    def forward(self, input_embedding):
        ...
        weight = torch.index_select(
            self.weight, dim=0, index=self.expand_index.int()
