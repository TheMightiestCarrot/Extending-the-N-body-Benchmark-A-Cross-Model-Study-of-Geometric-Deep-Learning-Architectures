...
def invariant_attr_rn(rel_pos):
    dists = rel_pos.norm(dim=-1, keepdim=True)
    return dists


def invariant_attr_r3s2_fiber_bundle(
    ori_grid,
    rel_pos,
    separable=False,
):
...
def invariant_attr_r2s1_point_cloud(pos, edge_index):
    ...
    invariant1 = (
        rel_pos[..., 0] * ori_receive[..., 0] + rel_pos[..., 1] * ori_receive[..., 1]
    )
    invariant2 = (
        -rel_pos[..., 0] * ori_receive[..., 1] + rel_pos[..., 1] * ori_receive[..., 0]
    )
