...
from models.CGENN.algebra.norms import cayley
...
class SteerableGeometricProductLayer(nn.Module):
    def __init__(
        ...
        # Register metric as a buffer so it moves with the model
        self.register_buffer('metric', metric if torch.is_tensor(metric) else torch.tensor(metric))
    ...
    def _get_weight(self):
        ...
        return (
            cayley(self.algebra, torch.linalg.eig(self.metric)[0].real)
            * weight_repeated
        )
