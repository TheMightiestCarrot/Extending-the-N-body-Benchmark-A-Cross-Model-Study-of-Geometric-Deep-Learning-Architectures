...
from models.CGENN.algebra.norms import calculate_norms, qs
...
class MVSiLU(nn.Module):
    ...
    def _norms_except_scalar(self, input):
        return calculate_norms(
            self.algebra,
            torch.linalg.eig(self.metric)[0].real,
            input,
            grades=self.algebra.grades[1:].to(input.device),
        )
