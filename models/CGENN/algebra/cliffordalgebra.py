...
from models.CGENN.algebra.metric import (
...
class CliffordAlgebra(nn.Module):
    def __init__(self, metric, device=None):
        ...
        # If no device specified, use CPU (will be moved later with model.to(device))
        if device is None:
        ...
        cayley = (
            construct_gmt(
                self.bbo.index_to_bitmap,
                self.bbo.bitmap_to_index,
                self.metric,
        ...
        self.register_buffer(
            "subspaces",
            torch.tensor(tuple(math.comb(self.dim, g) for g in self.grades), device=device),
        )
        self.n_subspaces = len(self.grades)
        self.grade_to_slice = self._grade_to_slice(self.subspaces)
        self.grade_to_index = [
            torch.tensor(range(*s.indices(s.stop)), device=device) for s in self.grade_to_slice
        ]
