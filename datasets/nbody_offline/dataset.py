...
class NBodySystemDataset(Dataset):
    ...
    def get_graph_step(self, loc_0, vel_0, charges, loc_t, vel_t):
        ...
        edge_attr = torch.norm(
            loc_0[edge_index[0], :] - loc_0[edge_index[1], :], p=2, dim=1
        ).unsqueeze(-1)
        ...
        node_feat = torch.cat(
            [feat_node_velocity, feat_node_charge / feat_node_charge.max()], dim=1
        )

        # Virtual node loc = mean
        loc_mean = (
            torch.mean(loc_0, dim=0)
            .unsqueeze(-1)
            .repeat(1, self.virtual_channels)
            .unsqueeze(0)
        )  # [1, 3, C]
    ...
    def cutoff_edge(self, loc_0):
        ...
        dist += (
            torch.eye(num_node_r).to(loc_0.device) * 1e18
        )  # [num_node_r, num_node_r]
        num_edge_rr_chosen = int(num_node_r * (num_node_r - 1) * (1 - self.cutoff_rate))
        _, id_chosen = torch.topk(
            dist.view(num_node_r * num_node_r), num_edge_rr_chosen, dim=0, largest=False
        )
        edge_rr = torch.cat(
            [
